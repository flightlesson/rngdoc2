...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"
  ...
  <xsl:template mode="Simplify-4.05" match="/">
    ...
    <xsl:if test="$debug-level > 0">
      <xsl:message>Simplify-4.05: stop-after is <xsl:value-of select="$stop-after"/>, debug-level=<xsl:value-of select="$debug-level"/>, input-uri is <xsl:value-of select="$input-uri"/></xsl:message>
    </xsl:if>
    ...
    <xsl:if test="$debug-level &gt; 1">
      <redirect:write file="debug-Simplify-4.05{$input-uri}.xml">
